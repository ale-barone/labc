...
from LatticeAB.data import dataAnalysis as dt
...
def fit(x, data_in, fit_function, func_args=None, rangefit=None, thin=1, guess = [1, 1], correlated=False):
    ...
    if num_param==1:        
        ...
        return dt.data_stats(out, data_in.stats_type, data_in.stats_par)
    else:
        for p in range(num_param):
            mean_p = np.array([fit_mean[p]])
            err_p = np.array([fit_err[p]])
            ...
            out_aux = dt.data_stats(out_aux, data_in.stats_type, data_in.stats_par)
            out.append(out_aux)     
